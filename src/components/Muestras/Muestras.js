...
function Muestras() {
    ...
    const enviarAAutorizar = () => {
        ...
        if (!lastSolicitud.enviadoParaAutorizar) {
            ...
            const updatedSolicitudes = [...solicitudes.slice(0, lastSolicitudIndex), lastSolicitud];
            guardarSolicitudes(updatedSolicitudes);
    ...
    const removeProduct = (codigo) => {
        ...
        const productosActualizados = productosModal.filter(item => item.codigo !== codigo);
        ...
        const updatedSolicitud = { ...currentSolicitud, carrito: productosActualizados };
        const updatedSolicitudes = [...solicitudes.slice(0, lastSolicitudIndex), updatedSolicitud];
        guardarSolicitudes(updatedSolicitudes);

        // También actualizamos el número de artículos en la solicitud
        const cantidadArticulos = productosActualizados.length;
        const updatedSolicitudWithCount = { ...updatedSolicitud, cantidadArticulos };
        const updatedSolicitudesWithCount = [...solicitudes.slice(0, lastSolicitudIndex), updatedSolicitudWithCount];
                                                ...
                                                <TableRow>
                                                    ...
                                                    <TableCell>
                                                        <Grid container spacing={2} alignItems="center">
                                                            <Grid item>
                                                                <IconButton
                                                                    onClick={() => setCantidad(cantidad > 1 ? cantidad - 1 : 1)} // Reduce la cantidad, pero nunca menos de 1
