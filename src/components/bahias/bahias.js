...
function Bahias() {
  ...
  const fetchBahias = () => {
    fetch('http://192.168.3.27:3007/api/bahias/bahias')
      .then(response => {
        if (!response.ok) {
  ...
  const handleClickOpen = (bahia) => {
    ...
    if (/^B-/.test(bahia.bahia)) {
      Swal.fire('Error', 'Esta bahía está bloqueada y no se puede liberar.', 'error');
  ...
  const handleConfirm = (bahia) => {
    fetch(`http://192.168.3.27:3007/api/bahias/liberar/${bahia.id_bahia}`, {
      method: 'PUT',
      headers: {
      ...
      .then(data => {
        ...
        setBahias(prevBahias => {
          ...
          updatedBahias[letter] = updatedBahias[letter].map(b =>
            b.id_bahia === bahia.id_bahia ? { ...b, estado: null, id_pdi: null } : b
          );
      ...
      .catch(error => {
        ...
        Swal.fire('Error!', 'Hubo un problema al liberar la ubicación.', 'error');
      });
  };
  ...
  if (error) {
    return <Box display="flex" justifyContent="center" alignItems="center" height="100vh"><Typography color="error">Error: {error.message}</Typography></Box>;
  }
    ...
    <Box sx={{ overflowX: 'auto', padding: 2 }}>
      ...
      <center>
        <Grid container spacing={1} justifyContent="center">
          {Object.keys(bahias).sort().map(letter => (
              ...
              <Grid container direction="column" alignItems="center" spacing={1}>
                {bahias[letter].map(bahia => (
                  <Grid item key={bahia.id_bahia}>
                    <Card 
                      style={{ width: 100, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', ...getCardStyle(bahia) }} 
                      onClick={() => handleClickOpen(bahia)}
                    >
                      <CardContent style={{ textAlign: 'center' }}>
                        <Typography variant="body2" component="div"  style={{ fontSize: '1em' }}>
                          {bahia.bahia}
                        </Typography>
                        ...
                        <Typography variant="body2" color="textSecondary" style={{ fontSize: '1em' }}>
                        {bahia.ingreso ? new Date(bahia.ingreso).toLocaleDateString() : 'Sin Ingreso'}
                      </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
