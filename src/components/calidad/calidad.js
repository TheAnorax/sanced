...
function Calidad({ onCloseModal }) {
  ...
  const fetchCalidadData = async () => {
    try {
      const response = await axios.get("http://192.168.3.27:3007/api/calidad/calidad");
  ...
  const handleAutorizar = async (recibo) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.put("http://192.168.3.27:3007/api/calidad/calidad/autorizar", {
  ...
  const handleCuarentena = async (recibo) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.put("http://192.168.3.27:3007/api/calidad/calidad/cuarentena", {
            codigo: recibo.codigo,
            oc: recibo.oc,
            cantidad_recibida: recibo.cantidad_recibida,
            fecha_recibo: recibo.fecha_recibo,
  ...
  const handleSegundas = async (recibo) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.put("http://192.168.3.27:3007/api/calidad/calidad/segundas", {
            codigo: recibo.codigo,
            oc: recibo.oc,
            cantidad_recibida: recibo.cantidad_recibida,
            fecha_recibo: recibo.fecha_recibo,
  ...
  const fetchProductoDetalles = async (codigo) => {
    setLoading(true);
    try {
      const response = await axios.post("http://192.168.3.27:3007/api/calidad/calidad/codigo", { codigo_pro: codigo });
  ...
  const handleSave = async () => {
    try {
      await axios.put("http://192.168.3.27:3007/api/calidad/calidad/updatecodigo", {
        ...productoDetalles,
  ...
  const handleSearchProduct = async (e) => {
    if (e.key === "Enter" && searchCode.trim() !== "") {
      try {
        const response = await axios.post("http://192.168.3.27:3007/api/calidad/calidad/codigo", { codigo_pro: searchCode });
        setProductoDetalles(response.data);
      } catch (error) {
  ...
  const handleSaveNewProduct = async () => {
    try {
      const response = await axios.post("http://192.168.3.27:3007/api/calidad/calidad/insertarBuscar", {
        ...productoDetalles,
  ...
  const fetchInactiveProducts = async () => {
    try {
      const response = await axios.get("http://192.168.3.27:3007/api/calidad/calidad/activoinactivo");
