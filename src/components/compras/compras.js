...
function Compras() {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios.put(
          `http://192.168.3.154:3007/api/compras/compras/cancelar/${id_recibo}`, // URL corregida
  ...
  const handleGuardarCantidad = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios.put(
          `http://192.168.3.154:3007/api/compras/compras/recibo/${selectedProduct.id_recibo}`,
          {
  ...
  const handleAssignToAlmacen7050 = async (compra) => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const response = await axios.put(
          `http://192.168.3.154:3007/api/compras/compras/recibo/${compra.id_recibo}`,
          {
  ...
  const fetchCompras2 = async () => {
    try {
      const response = await axios.get(
        `http://192.168.3.154:3007/api/compras/compras?tipo=${tipo}`
      );
  ...
  useEffect(() => {
    if (tipo) {
      ...
      const fetchProductos = async () => {
        try {
          const response = await axios.get(
            "http://192.168.3.154:3007/api/productos"
          );
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        for (const batch of batches) {
          await axios.post(
            "http://192.168.3.154:3007/api/compras/compras/upload-excel",
            batch
          );
        }
  ...
  //   try {
  //     await axios.post(
  //       "http://192.168.3.154:3007/api/compras/compras/upload-pdfsOC",
  //       formData,
  //       {
  ...
  const handleFilesUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.3.154:3007/api/compras/compras/upload-pdfs",
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
          onUploadProgress: (progressEvent) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        if (isEditing) {
          await axios.put(
            `http://192.168.3.154:3007/api/compras/recibo/${selectedProduct.id_recibo}`,
            {
              oc,
              cantidad,
          ...
          const userId = user?.id_usu;
          await axios.post("http://192.168.3.154:3007/api/compras/recibo", {
            oc,
            cantidad,
            recepcion: recepcionFormatted,
            codigo: selectedProduct.codigo_pro,
          ...
          >
            ...
            <Grid item xs={2}>
              ...
              {detalleData?.pdf_1 ? (
                <div>
                  <Button
                    href={`http://192.168.3.154:3011/docs/${detalleData.pdf_1}`}
                    target="_blank"
              ...
              {detalleData?.pdf_2 ? (
                <div>
                  {detalleData.pdf_2.split(",").map((pdfName, index) => (
                    <Button
                      key={index}
                      href={`http://192.168.3.154:3011/docs/${pdfName.trim()}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      variant="contained"
                      sx={{
            ...
            <Grid item xs={2}>
              ...
              {detalleData?.pdf_3 ? (
                <div>
                  <Button
                    href={`http://192.168.3.154:3011/docs/${detalleData.pdf_3}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    variant="contained"
                    sx={{
            ...
            <Grid item xs={2}>
              ...
              {detalleData?.pdf_5 ? (
                <div>
                  <Button
                    href={`http://192.168.3.154:3011/docs/${detalleData.pdf_5}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    variant="contained"
                    sx={{
