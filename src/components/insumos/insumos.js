...
import {
    Box, TextField, useMediaQuery, createTheme, ThemeProvider, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Button, Snackbar, Alert, Tabs, Tab, Typography, Modal, TableCell, TableContainer, TableHead,
    TableRow, Paper, Table, TableBody, Grid,
} from "@mui/material";
...
function Insumos() {
    ...
    const fetchInsumos = async () => {
        try {
            ...
            setInsumos(data.map((insumo, index) => ({ id: index, ...insumo })));
            setFilteredInsumos(data.map((insumo, index) => ({ id: index, ...insumo })));
        } catch (error) {
    ...
    const generateLowInventoryReport = (observationText) => {
        const lowInventoryByConsumption = insumos.filter((insumo) => {
            ...
            if (isNaN(requerimiento)) {
                console.warn(`Valor no válido para el producto ${insumo.id_codigo} (requerimiento: ${insumo.requerimiento})`);
                return false; 
            }
        ...
        console.log("Productos con inventario menor al consumo mensual o inventario en 0:", lowInventoryByConsumption);
    
        if (lowInventoryByConsumption.length === 0) {
            alert("No hay productos cuyo inventario sea menor al consumo mensual o en 0.");
            return;
        }
                                ...
                                <table>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td colspan="4">${user.name} - ${user.email}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">Observaciones</td>
                                            <td colspan="4">${observationText || "Sin Observaciones"}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            <div class="main-content">
                                <table>
                                    <tbody>
                                        ...
                                        ${lowInventoryByConsumption.map((producto) => `
                                            <tr>
    ...
    const handleUpdate = async () => {
        ...
        const requestBody = { codigopropuesto, ...dataToUpdate };

        console.log("Insumo a actualizar:", selectedInsumo);
    ...
    const handleFilterToggle = () => {
        ...
        document.getElementById('insumo-container').classList.toggle('scrollable', !isFiltered);
      };
            ...
            <Box
                ...
                <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
                    ...
                    <Dialog open={isModalOpen} onClose={handleCloseModal}>
                        ...
                        <DialogActions>
                            <Button onClick={() => { setIncludeObservation(false); handleObservationSubmit(); }} color="primary">
                                No
                            </Button>
                            <Button onClick={() => setIncludeObservation(true)} color="primary">
                                Sí
                            </Button>
                        </DialogActions>
