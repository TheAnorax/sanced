...
function Inventory() {
  ...
  const handleGenerateExcelReports = async () => {
    try {
      ...
      const [pickResponse, almacenajeResponse, consolidatedResponse] =
        await Promise.all([
          axios.get(
            "http://192.168.3.27:3007/api/inventory/reportFinishInventory"
          ),
          axios.get(
            "http://192.168.3.27:3007/api/inventory/reportFinishInventoryAlma"
          ),
          axios.get(
            "http://192.168.3.27:3007/api/inventory/reportConsolidatedInventory"
          ),
        ]);
  ...
  const fetchInventory = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        "http://192.168.3.27:3007/api/inventory/obtenerinventario"
      );
  ...
  const handleSaveClick = async () => {
    try {
      ...
      const response = await axios.put(
        `http://192.168.3.27:3007/api/inventory/update/${editRowId}`,
        editValues
      );
