...
function EnSurtido() {
  ...
  const updateBahiasFromModal = async () => {
    try {
      ...
      const response = await axios.put(
        `http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido/${pedidoBahiaSeleccionado.pedido}/bahias`,
        {
    ...
    const fetchPedidos = async () => {
      try {
        const response = await axios.get(
          "http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido"
        );
  ...
  const fetchTurnoPedidos = async (turno) => {
    try {
      const response = await axios.get(
        "http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-dia"
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido/${selectedPedido.pedido}`,
        updatedPedido
      );
      const updatedPedidos = pedidos.map((pedido) =>
  ...
  const updateAllBahias = async () => {
    try {
      ...
      const response = await axios.put(
        `http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido/${selectedPedido.pedido}/bahias`,
        {
          ubi_bahia: combinedBahias,
        }
      );
  ...
  console.log("📄 Tipo:", selectedPedido.tipo);
      ...
      await axios.post(
        `http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido/${selectedPedido.pedido}/authorize`,
        {
  ...
  const handleCancelPedido = async (pedidoId) => {
    try {
      const response = await axios.put(
        `http://192.168.3.154:3007/api/pedidos-surtidos/pedidos-surtido/${pedidoId}/cancel`,
        { estado: "C" }
      );
      if (response.status === 200) {
        const updatedPedidos = pedidos.map((pedido) => {
