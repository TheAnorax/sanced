...
function Recibo() {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      await axios.post("http://192.168.3.154:3007/api/recibo/guardarRecibo", {
  ...
  const fetchRecibos = async (dato) => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.3.154:3007/recibo/lista",
        {
          dato: dato,
        }
      );
  ...
  const handleRecibir = async (recibo) => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.3.154:3007/recibo/tarima",
        { 
  ...
  const handleGuardarDatos = async () => {
    ...
    try {
      ...
      if (confirmacion.isConfirmed) {
        await axios.post(
          "http://192.168.3.154:3007/api/recibo/guardarTarima",
          editedTarimaData
        );
        Swal.fire(
          "¡Guardado!",
