...
import { Checkbox, Dialog, Divider, Select, TextField, Typography, Box, FormControl, InputLabel, MenuItem, FormControlLabel, DialogTitle, DialogContent, DialogActions, Card, CardContent, Avatar, IconButton, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Alert, Snackbar, Grid, Tooltip, ButtonGroup, Menu,} from "@mui/material";
import { PhotoCamera, CloudUpload, CarCrash, Search, PersonAddAlt, LocalShipping, Add, PrecisionManufacturing, UploadFile, Schedule, Clear, Backup, DoNotDisturbOn, CheckCircleOutline, DirectionsCarFilled, AccessAlarm, EventAvailableOutlined, Error, AvTimer, DeleteOutline, Autorenew, Business, Person, CarCrashOutlined, CloudQueue, UploadFileOutlined, HelpOutline,ErrorOutline, DirectionsSubway, BusAlert, EditCalendar, SensorOccupied, ConnectWithoutContact,AirlineSeatReclineNormal, DirectionsCar, EventBusy, FmdGoodOutlined, EngineeringOutlined, KeyboardArrowDown, BlockOutlined, Block, Edit, PersonPin, } from "@mui/icons-material";
...
function Visitantes() {
  ...
  const [selectedCortina2, setSelectedCortina2] = useState(null);
  ...
  const horariosManana = useRef([]);
  const horariosTarde = useRef([]);
  const horariosNoche = useRef([]);
  const ejecutados = useRef(new Set()); // Horarios ya ejecutados

  const getHorarios = async () => {
    try {
      const response = await axios.get(`${api}/alert`);
      console.log("Horarios:", response.data.manana);

      horariosManana.current = response.data.manana;
      horariosTarde.current = response.data.tarde;
      horariosNoche.current = response.data.noche;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      ...
      const todosHorarios = [
        ...horariosManana.current,
        ...horariosTarde.current,
        ...horariosNoche.current,
      ];

      if (todosHorarios.includes(horaActual) && !ejecutados.current.has(horaActual)) {
        generarCodigo();
        ejecutados.current.add(horaActual);
        localStorage.setItem("horariosEjecutados", JSON.stringify([...ejecutados.current]));
      }
    }, 1000 * 60); // Cada minuto
  ...
  const cancelarVisita = async (idVisit) => {
    try {
  ...
  const convertirATextoMayusculas = (obj) => {
    const nuevoObjeto = {};
      for (const [key, value] of Object.entries(obj)) {
        nuevoObjeto[key] = typeof value === "string" ? value.toUpperCase() : value;
      }
    return nuevoObjeto;
  };
...
//separar la validaciones
  const validateVisitante = () => { 
    ...
    if (!(invit.apellidos || '').trim()) {
      ...
      !/^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)+$/.test(invit.apellidos.trim())
    ) {
    ...
    if(checked){
      ...
      if (!(invit.placa || '').trim()) {
        ...
        isValid = false;
      } else if (!/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,7}$/i.test(invit.placa)) {
  ...
  const validateVehiculo = () => { 
    ...
    if (!(vehiculo.placa || '').trim()) {
      ...
      isValid = false;
    } else if (!/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,7}$/i.test(vehiculo.placa)) {
  ...
  const handleClickCloseEntrevista = () => {
    setOpenCreateEntrevista(false);
    setPaq({
      id_catv: '', 
      nombre: '',
      apellidos: '',
  ...
  const handleClickOpenCortina = (visita) => {
    console.log('El objeto:', visita);
    if (!visita.clave) {
        console.error('El objeto visita no contiene el campo clave:', visita);
        return;
    }
  ...
  const validateVisitaAcomp = () => { 
    ...
    if (!vitAcomp.apellidos_acomp || !vitAcomp.apellidos_acomp.trim()) {
      ...
      !/^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)+$/.test(vitAcomp.apellidos_acomp.trim())
    ) {
  ...
  const createVisitaPaqueteria = async () => {
    ...
    if (['PAQUETEXPRESS', 'FLECHISA', 'PITIC', 'TRES GUERRAS', 'FEDEX', 'TRANSPORTES ROMERO', 'TRANSPORTES VEGA' ,'TANSPORTES CARRANZA', 'TANSPORTES LEAL', 'TANSPORTES LS',
      ...
      id_catv = 13;
    }else if(['CARTON','CLIENTE RECOGE'].includes(paq.empresa)){
      id_catv = 11;
    }

    if (paq.empresa === 'OTRO') {
      empresa = paq.empresaFin;
    }
  ...
  const updateVisitaPaqueteria = async () => {
    if (!upPaq.area_per || upPaq.area_per === 0) {
      setErrorVisitaPaqueteria((prev) => ({
        ...prev,
        area_per: 'Selecciona una cortina.',
      }));
    ...
    const data = {
      ...upPaq,
      clave_visit: selectedVisita.clave_visit,
    }
    Swal.fire({
      title: 'Espere...',
      text: 'Estamos guardando la información',
  ...
  const validateVisitantePaqueteria = () => { 
    ...
    if (!(paq.apellidos || '').trim()) {
      ...
      !/^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+(?:\s+[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+)+$/.test(paq.apellidos.trim())
    ) {
      validationErrors.apellidos = "Debes ingresar los dos apellidos.";
      isValid = false;
    }
    ...
    // if(selectedPaqueteria.id_paq ==! 8) {
      ...
      if (!(paq.placa || '').trim()) {
        validationErrors.placa = "Este dato es obligatorio.";
        isValid = false;
      } else if (!/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,7}$/i.test(paq.placa)) {
        validationErrors.placa = "Debe tener 6 caracteres, incluyendo letras y números.";
        isValid = false;
      }
    // }

    if(selectedPaqueteria.id_paq === 20) {
      if(!(paq.empresaFin || '').trim()){
        validationErrors.empresaFin = "Este dato es obligatorio.";
        isValid = false;
      }
    }
  ...
  const handleDropdownChange = (event) => {
    const selectedValue = event.target.value;
    setSelectedAcs(selectedValue);
    setVisita((prevState) => ({
      ...prevState,
      id_vit: selectedValue?.clave || "",
      id_veh: selectedValue?.id_veh || '',
    }));
  };
  ...
  const validarVisita = async (idVisit) => {
    try {
      const idV = idVisit.id_visit;
      const response = await axios.put(`${api}/llegada/${idV}`,{ llegada: 'S'}, { 
          headers: { 'Content-Type': 'application/json', }
        }
      );
      window.location.reload();
  ...
  const validar = async (idVisit) => {
    try {
      const idV = idVisit.id_visit;
      const response = await axios.put(`${api}/validar/${idV}`, { validar: 'S',}, {
          headers: { 'Content-Type': 'application/json', },
        }
      );
      window.location.reload();
  ...
  const validProv = [
    "PROVEEDOR (IMPORTACIONES/NACIONALES)", "PROVEEDOR"
  ];
  ...
  const handleDropdownChangeCortina2 = (event) => {
    const selectedValue = event.target.value;
    setSelectedCortina2(selectedValue);
    setPaq((prevState) => ({
      ...prevState,
      area_per2: selectedValue?.id_cor || "",
      motivo2: selectedValue?.area || "",
    }));
  }; 

  const handleDropdownChangeUpCortina = (event) => {
    const selectedValue = event.target.value;
    setSelectedCortina(selectedValue);
    setUpPaq((prevState) => ({
      ...prevState,
...
//responsive filtros
  ...
  return (
    <div>
            ...
            {(user?.role === 'POLIB' || user?.role === 'Admin' || user?.role === 'Master') && (
              <>
                <TextField
                  ...
                  size="small"
                  type="search"
                  sx={{ width: isMobile ? 200 : 300, mb: 2, }}
                  InputProps={{ 
                    endAdornment: (
                      <>
                        {filteredVisitas.length > 0  ? (
                          <IconButton aria-label="clear" size="small" onClick={handleClearSearch} >
                    ...
                    endAdornment: (
                      <>
                        {filteredEmpleados.length > 0  ? (
                          ...
                          <IconButton aria-label="search" size="small" onClick={handleSearchEmpleado} >
                            <Search fontSize="small" />
                          </IconButton>
                        )}
                      </>
                  ...
                  endAdornment: (
                    <>
                      {filteredPlacas.length > 0  ? (
                        ...
                        <IconButton aria-label="search" size="small" onClick={handleSearchPlaca} >
                          <Search fontSize="small" />
                        </IconButton>
                      )}
                    </>
              ...
              <Grid item xs={12} sm={6} md={4.5} key={index}>
                <Card sx={{ height: '100%', width: '120%', display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <CardContent sx={{ display: "flex" }}>
                    <Avatar src={`${foto}/${empleado.foto}`} alt={empleado.nombre_completo} sx={{ width: 150, height: 150, marginRight: "15px", objectFit: "cover" }}/>
                    <div style={{ flex: 1 }}>
            ...
            <Grid item xxs={12}>
              <Typography variant="h6" color="textSecondary" align="left" sx={{color:'red'}}>
                {searchQuery.trim() !== "" && !searchPerformed
                  ? <strong style={{  margin: '8px' }}>Por favor, ingresa el nombre completo para realizar la búsqueda.</strong>
                  : <strong style={{  margin: '8px' }}>No se encontró ninguna visita.</strong>}
              </Typography>
            </Grid>
          ) : null}
        </Grid>
          ...
          {filteredPlacas.length > 0 ? (
            filteredPlacas
              ...
              .map((visita, index) => (
                <Grid item xs={12} sm={6} md={4.5} key={index}>
                  <Card
                    sx={{ height: '100%', width: '120%', display: 'flex', flexDirection: 'column', }} >
                    <CardContent sx={{ display: 'flex' }}>
                      <Avatar src={`${foto}/${visita.foto}`} alt={visita.nombre_completo} sx={{ width: 150, height: 150, marginRight: '15px', objectFit: 'cover' }} />
                      {visita.id_catv === 4 ? (
                        <div style={{ flex: 1 }}>
                          ...
                          <div>
                            <Button variant="outlined" startIcon={<BusAlert />} sx={{ width: '90%', mb: 1 }} onClick={() => validar(visita)} >
                              Validar acceso
                            </Button>
                          </div>
                        </div>
            ...
            {visitas.filter((vit) => vit.llegada === 'S' && vit.entrada_h === null).map((vit, index) => (
              <Grid item xs={12} sm={6} md={4.5} key={index}>
                <Card sx={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', }} >
                  <CardContent sx={{ display: 'flex' }}>
                    <Avatar src={`${foto}/${vit.foto}`}alt={vit.nombre_completo} sx={{ width: 150, height: 150, marginRight: '15px', objectFit: 'cover' }} />
                    <div style={{ flex: 1 }}>
                      ...
                      <Typography variant="body2" sx={{ marginBottom: 1 }}>
                        <Business fontSize="small" /> 
                          Empresa: {
                            // vit.id_catv === 1 ? 'COLABORADOR SANTUL'
                      ...
                      <div>
                        {vit.foto === null || vit.foto === '' ? (
                          vit.id_catv === 4 ? (
                            <Button variant="outlined" startIcon={<PhotoCamera />} sx={{ width: "90%", mb: 1 }} onClick={() => handleClickOpenCamProv(vit)} >
                              Tomar Foto
                            </Button>
                          ):(
                            (vit.foto !== null || vit.foto === '' &&  vit.entrada_h === null  ? (
                              <>
                                <Button style={{marginTop:'8px'}} size="small" variant="outlined" color="warning" startIcon={<i className="pi pi-qrcode" />} onClick={() => handleClickOpenAcceso(vit)} >
                                  Pase de acceso
                                </Button>
                                <Button style={{ marginTop: '8px' }} size="small" variant="outlined" color="warning" startIcon={<i className="pi pi-qrcode" />} onClick={() => handleClickOpenRegAcomp(vit)} >
                                  Registrar acompañante
                                </Button>
                              </>
                            ): (
                            <Button variant="outlined" startIcon={<PhotoCamera />} sx={{ width: "90%", mb: 1 }} onClick={() => handleClickOpenCamUp(vit)} >
                              Tomar Foto
                            </Button>
                          ))
                        )
            ...
            {visitas.filter((vit) =>  vit.id_catv !== 12 && ( vit.placa === '' || vit.img1 === '' ||  vit.validado === '' || vit.validado === '' || vit.validado === null) && (vit.entrada_h !== null)).map((vit, index) => (
              <Grid item xs={12} sm={6} md={4.5} key={index}>
                <Card  className="card__responsive" sx={{ height: '100%', width: '100%', flexDirection: 'column', }}  >
                  <CardContent sx={{ display: 'flex'  }}>
                    <Avatar src={`${foto}/${vit.foto}`} alt={vit.nombre_completo} sx={{ width: isMobile ? 65 : 150, height: isMobile ? 65 : 150, marginRight: isMobile ? 1 : '15px', objectFit: 'cover', }}/>
                    <div style={{ flex: 1 }}>
                      {vit.catv === '4' || vit.tipo === 'PROVEEDOR (IMPORTACIONES/NACIONALES)' ? (
                      ...
                      <Typography variant="body2" sx={{ marginBottom: 1 }}>
                        <Business fontSize="small" /> 
                          Empresa: {
                            ...
                            //   : vit.id_catv === 7 ? vit.paqueteria
                            //   : vit.id_catv === 2 || vit.id_catv === 4 ? 'N/A'
                            //   : vit.empresa === '6' || vit.empresa === '8' ? 'N/A'
                            //   : 
                              vit.empresa
                          }
                      </Typography>
        ...
        <DialogContent>
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2, justifyItems: 'center', textAlign: 'center', width: '100%', }} >
            {Object.entries(images).map(([key, value], index) => (
              <Box key={index} sx={{ width: 230, height: 240, border: '1px dashed #ccc', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', }} >
                {value ? (
                  <Box sx={{ textAlign: 'center' }}>
                    <img src={value} alt={`Foto ${index + 1}`} style={{ width: 200, height: 200, objectFit: 'cover', borderRadius: 8, }} />
                    <Typography variant="body2" sx={{ color: 'black' }}>
                      {imageLabels[key]} {/* Mostrar la descripción según la clave */}
                    </Typography>
                    <Button variant="contained" color="error" size="small" onClick={() => removeImageVehiculo(index)} sx={{ position: 'absolute', top: 5, right: 5 }} >
                      Quitar
                    </Button>
                  </Box>
                ) : (
                  <Box>
                    <input accept="image/*" type="file" style={{ display: 'none' }} id={`file-input-${index}`} onChange={(e) => handleFileChange(e, index)} />
                    <label htmlFor={`file-input-${index}`}>
                      <Button variant="outlined" startIcon={<PhotoCamera />} sx={{ width: '100%' }} component="span" >
          ...
          <FormControl fullWidth>
            <TextField
              ...
              sx={{ width: '100%', marginTop: 2, marginX: 'auto', }}
              ...
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: "1px dashed #ccc", width: "100%", height: 230, textAlign: "center", margin: "auto", }} >
                {image ? (
                  <Box sx={{ textAlign: "center", width: "100%", height: "100%" }} >
                    ...
                    <Button variant="contained" color="error" size="small" onClick={removeImage} sx={{ mt: 1 }} >
                      Quitar Imagen
                    </Button>
                  </Box>
                ) : (
                  <Box>
                    <Button variant="outlined" startIcon={<CloudUpload />} sx={{ width: "90%", mb: 1 }} component="label" >
                      Seleccionar Foto
                      <input type="file" accept="image/*" onChange={onTemplateSelect1} hidden />
                    </Button>
                    <Button variant="outlined" startIcon={<PhotoCamera />} sx={{ width: "90%", mb: 1 }} onClick={handleClickOpenCam} >
                      Tomar Foto
                    </Button>
      ...
      <Dialog open={openCreateEntrevista} onClose={handleClickCloseEntrevista} maxWidth="md" fullWidth >
        <DialogTitle>NUEVO VISITANTE (ENTREVISTA)</DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} >
              <FormControlLabel
                control={ <Checkbox checked={checkedEnt} onChange={(e) => setCheckedEnt(e.target.checked)} /> }
                label="Acompañantes"
              />
              {checkedEnt && (
                <Grid item xs={12}>
      ...
      <Dialog open={openCreateTransp} onClose={handleCloseCreateTransp} maxWidth="md" fullWidth >
        <DialogTitle> 
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={3} flexDirection={isSmallScreen ? "column" : "row"}>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={3} flexDirection={isSmallScreen ? "column" : "row"} marginTop={3}>
                    ...
                    <Grid item marginTop={2} sm={2} md={3.5}>
                      ...
                      <label htmlFor="upload-excel">
                        <Button variant="outlined" endIcon={<UploadFile />} component="span" fontSize="small" >
                          Seleccionar archivo
                        </Button>
                      </label>
                    </Grid>
                ...
                <DialogContent>
                  {dataExcel.length > 0 ? (
                        ...
                        <TableHead>
                          <TableRow>
                            {Object.keys(dataExcel[0]).map((key) => (
                              <TableCell key={key}
                              style={{ backgroundColor: invalidColumns.includes(key) ? "#f8d7da" : "inherit", color: invalidColumns.includes(key) ? "#721c24" : "inherit", }}>{key}
                              </TableCell>
                            ))}
                          </TableRow>
                        </TableHead>
                    ...
                    {errorExcel.length > 0 ? (
                      ...
                      <p> <Alert severity="success">Archivo correcto.</Alert> </p>
                    )}
                    </>
                  ) : (
                    <Grid>
            ...
            <Dialog open={openUpExcelVeh} onClose={handleCloseUpExcelVeh} maxWidth="md" fullWidth >
              <Box >
                <DialogTitle>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={3} flexDirection={isSmallScreen ? "column" : "row"} marginTop={3} >
                    <Typography variant="h6" gutterBottom>
                    ...
                    <Box display="flex" flexDirection="column" gap={2} marginTop={2}>
                      <Grid item sm={12}>
                        <input type="file" accept=".xlsx, .xls" onChange={handleFileUploadVeh} style={{ display: "none" }} id="upload-excel" />
                        <label htmlFor="upload-excel">
                          <Button variant="outlined" endIcon={<UploadFile />} component="span" fontSize="small" >
                            Seleccionar archivo
                          </Button>
                      ...
                      <Grid item sm={12}>
                        <Button variant="outlined" endIcon={<DirectionsCar />} component="span" fontSize="small" onClick={handleOpenNewVeh} >
                          Registrar vehículo
                        </Button>
                      </Grid>
                    </Box>
                        ...
                        <TableHead>
                          <TableRow>
                            {Object.keys(dataExcelVeh[0]).map((key) => (
                              <TableCell key={key} style={{ backgroundColor: invalidColumns.includes(key) ? "#f8d7da" : "inherit", color: invalidColumns.includes(key) ? "#721c24" : "inherit", }}>{key}</TableCell>
                            ))}
                          </TableRow>
                        </TableHead>
                        <TableBody>
                ...
                <DialogActions>
                  <Box>
                    {dataExcelVeh.length > 0 && (
                      <Box sx={{ textAlign: "center", mt: 4 }}>
                        <Button variant="outlined" color="primary" onClick={handleClearExcelVeh} sx={{ mr: 2 }}>
                          Cancelar 
                        </Button>
                        <Snackbar open={openAlert} autoHideDuration={6000} onClose={handleCloseSave}>
                          <Alert severity="error" onClose={handleCloseSave}>{errorSaveVeh}</Alert>
                        </Snackbar>
                        <Button variant="outlined" color="primary" endIcon={<Backup />} onClick={handleSaveVehiculoData} disabled={dataExcelVeh.length === 0} >
                          Guardar Datos
                        </Button>
                      </Box>
                    )}
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={3}>
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", border: "1px dashed #ccc", width: "100%", height: 230, textAlign: "center", margin: "auto", }} >
                {image ? (
                  <Box sx={{ textAlign: "center", width: "100%", height: "100%" }} >
                    <img src={ typeof image === "string" ? image : URL.createObjectURL(image) } alt="preview" style={{ width: "100%", maxHeight: 170, borderRadius: 8 }} />
                    <Typography variant="body2" sx={{ color: "black" }}>
                      {image.name || "Captura de Webcam"}
                    </Typography>
                    <Button variant="contained" color="error" size="small" onClick={removeImage} sx={{ mt: 1 }} >
                  ...
                  <Box>
                    ...
                    <Button variant="outlined" startIcon={<PhotoCamera />} sx={{ width: "95%", mb: 1 }} onClick={handleClickOpenCam} >
                      Tomar Foto
                    </Button>
                    <Dialog header="TOMAR FOTO" open={openCam} onClose={handleCloseCam} >
                      <Box style={{ textAlign: "center", margin:'20px'  }}>
      ...
      <Dialog open={openCreateVisita} onClose={handleCloseCreateVisita} maxWidth="md" fullWidth >
          ...
          <Grid container spacing={2}>
            <Grid item xs={4} sm={6} style={{marginTop:'5px'}}>
              ...
              <FormControl fullWidth sx={{ mb: 2 }}>
                <TextField
                  ...
                  value={visita.personal || (selectedAcs?.categoria === "TRANSPORTISTA" || selectedAcs?.categoria === "PAQUETERIA / TRANSPORTE" ? "ALEJANDRO GARCIA" : selectedAcs?.categoria === "ENTREGA DE EVIDENCIAS" ? "ROSA HERNANDEZ" : "")}
                  onChange={(e) => {
            ...
            <Grid item xs={12} sm={6} style={{marginTop:'5px'}}>
              <FormControl fullWidth sx={{ mb: 2 }}>
                <LocalizationProvider adapterLocale="es" dateAdapter={AdapterDayjs}>
                  <DatePicker label="Día de la visita" value={date} onChange={handleDateChange} format="DD-MM-YYYY" />
                </LocalizationProvider>
                <small>
                  {errorVisitas.reg_entrada && (
                    <span style={{color: 'red'}}>* {errorVisitas.reg_entrada}</span>
                  )}
                </small>
              </FormControl>
              <FormControl fullWidth sx={{ mb: 2 }}>
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <TimePicker label="Hora de la visita"value={time} onChange={handleTimeChange} renderInput={(params) => <TextField {...params} />} />
                </LocalizationProvider>
                <small>
                  {errorVisitas.hora_entrada && (
                    <span style={{color: 'red'}}>* {errorVisitas.hora_entrada}</span>
                  )}
                </small>
              </FormControl>
              <FormControl fullWidth sx={{ mb: 2 }}>
                <InputLabel id="area_per">Area que visita</InputLabel>
                  ...
                  >
                    ...
                    { 
                      (selectedAcs?.categoria === 'TRANSPORTISTA' || selectedAcs?.categoria === 'PAQUETERIA / TRANSPORTE' || selectedAcs?.categoria === 'MANIOBRISTA' ? areasTR : areas)
                      .map((item) => (
                  ...
                  <small>
                  {errorVisita.id_area && (
                    <span style={{color: 'red'}}>* {errorVisita.id_area}</span>
                  )}
                </small>
              </FormControl>
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth sx={{ mb: 2 }}>
                ...
                <small>
                  {errorVisitas.motivo && (
                    <span style={{color: 'red'}}>* {errorVisitas.motivo} </span>
                  )}
                </small>
              </FormControl>
              <FormControlLabel
                control={ <Checkbox checked={checkedVisita} onChange={(e) => setCheckedVisita(e.target.checked)} /> }
                label="Acompañantes"
              />
              {checkedVisita && (
                <Grid item xs={12}>
                ...
                {selectedPaqueteria?.id_paq === 20 && (
                  <FormControl fullWidth sx={{ mb: 2 }}>
                    <TextField
                      ...
                      name="empresaFin"
                      label="Empresa"
                      value={paq.empresaFin}
                      onChange={inputChangePaq}
                      inputProps={{ style: { textTransform: 'uppercase' } }}
                    />
                    {errorVisitaPaqueteria.empresaFin && (
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaPaqueteria.no_ide && (
                      <span style={{color: 'red'}}>* {errorVisitaPaqueteria.no_ide}</span>
                    )}
                  </small>
                </FormControl>
                {paq.no_ide && ( 
                  <>
                    <FormControlLabel
                      control={ <Checkbox size="small" checked={checkedIne} onChange={() => handleCheckboxChangeIne("ine")} /> }
                      label="No. INE"
                    />
                    <FormControlLabel
                      control={ <Checkbox size="small" checked={checkedLic} onChange={() => handleCheckboxChangeIne("lic")} /> }
                      label="No. Licencia"
                    /><br/>
                    <small>
                    {errorVisitaPaqueteria.checks && (
                      <span style={{ color: 'red' }}>* {errorVisitaPaqueteria.checks}</span>
                    )}
                    </small>
                  </>
                )}
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaPaqueteria.area_per && (
                      <span style={{color: 'red'}}>* {errorVisitaPaqueteria.area_per}</span>
                    )}
                  </small>
                </FormControl>
                <FormControl fullWidth sx={{ mb: 2 }}>
                  <InputLabel id="area_per2">No. Cortina 2 (*OPCIONAL)</InputLabel>
                  <Select
                    labelId="area_per2"
                    id="area_per2"
                    name="area_per2"
                    value={selectedCortina2}
                    onChange={handleDropdownChangeCortina2}
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaPaqueteria.marca && (
                      <span style={{color: 'red'}}>* {errorVisitaPaqueteria.marca}</span>
                    )}
                  </small>
                </FormControl>
              </Grid>
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaPaqueteria.placa&& (
                      <span style={{color: 'red'}}>* {errorVisitaPaqueteria.placa}</span>
                    )}
                  </small>
                </FormControl>
              </Grid>
            ...
            <TableContainer component={Paper} sx={{ maxHeight: 440 }}>
              <Table stickyHeader aria-label="sticky table">
                ...
                <TableHead>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} align="left" style={{ backgroundColor: '#f5f3f3', textAlign:'center' }}><strong>VISITANTES</strong></TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell align="center"></TableCell>
                    ...
                    return (
                      <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        <TableCell component="th" scope="row" align="center" sx={{ width: '30%' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <Avatar src={row.foto ? `${foto}/${row.foto}` : '/placeholder.png'} alt={row.nombre_completo || 'Sin nombre'} sx={{ width: 85, height: 85, marginRight: '15px', objectFit: 'cover' }} />
                ...
                <TableHead>
                  ...
                  <TableRow >
                    <TableCell colSpan={7} align="left" style={{ backgroundColor: '#f5f3f3', textAlign:'center' }}><strong>PAQUETERIAS</strong></TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell align="center"></TableCell>
                    <TableCell align="left">Nombre</TableCell>
                ...
                <TableHead>
                  ...
                  <TableRow >
                    <TableCell colSpan={7} align="left" style={{ backgroundColor: '#f5f3f3', textAlign:'center' }}><strong>CLIENTES</strong></TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell align="center"></TableCell>
                    <TableCell align="left" >Nombre</TableCell>
        ...
        {selectedVisitaBloqeo && (
          <div>
            <div style={{display:'flex', margin:'10px'}}>
              <div>
                <Avatar src={selectedVisitaBloqeo.foto ? `${foto}/${selectedVisitaBloqeo.foto}` : '/placeholder.png'} alt={selectedVisitaBloqeo.nombre_completo || 'Sin nombre'} sx={{ width: 150, height: 150, objectFit: 'cover', marginRight: '15px' }}/>
              </div>
              <div >
      ...
      <Dialog header="TOMAR FOTO" open={openCamVehiculo} onClose={handleCloseCamVehiculos} >
        <DialogContent>
          <Box style={{ textAlign: 'center', margin: '20px' }}>
            <Webcam
              audio={false}
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaAcomp.nombre_acomp && (
                      <span style={{color: 'red'}}>* {errorVisitaAcomp.nombre_acomp}</span>
                    )}
                  </small>
                </FormControl>
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <small>
                    {errorVisitaAcomp.apellidos_acomp && (
                      <span style={{color: 'red'}}>* {errorVisitaAcomp.apellidos_acomp}</span>
                    )}
                  </small>
                </FormControl>
                <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Box sx={{ textAlign: "center", width: "100%", height: "100%" }}>
              ...
              <div>
                ...
                {!isCameraReadyUp ? (
                  ...
                  <Button severity="danger" variant="outlined" startIcon={<PhotoCamera />} style={{ width: "100%", marginTop: "20px" }} onClick={tomarFotoUpdate} >
                    Tomar Foto
                  </Button>
                )}
              </div>
      ...
      <Dialog header="TOMAR FOTO" open={openCamUpDatos} onClose={handleCloseCamProv} >
        <DialogContent >
          {imageProv ? (
            <Box sx={{ textAlign: "center", width: "100%", height: "100%" }}>
              <img src={typeof imageProv === "string" ? imageProv : URL.createObjectURL(imageProv)} alt="preview" style={{ width: "100%", height: "150%", borderRadius: 8 }} />
              <Typography variant="body2" sx={{ color: "black" }}>
                {imageProv.name || "Captura de Webcam"}
              </Typography>
              <Button variant="outlined" color="error" size="small" onClick={removeImageProv} sx={{ mt: 1, margin:'10px',width: "50%" }} >
                Tomar de nuevo
              </Button>
            </Box>
            ):(
              <div>
                ...
                {!isCameraReadyUp ? (
                  ...
                  <Button severity="danger" variant="outlined" startIcon={<PhotoCamera />} style={{ width: "100%", marginTop: "20px" }} onClick={tomarFotoProveedor} >
                    Tomar Foto
                  </Button>
                )}
              </div>
      ...
      {/**DIALOG PARA ACTUALIZAR LA CORTINA */}
      <Dialog open={openCortina} onClose={handleCloseCortina}>
        <DialogTitle>Actualizar Cortina</DialogTitle>
        <DialogContent>
          <Divider textAlign="left">Información</Divider>
          <p/>
          <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', margin: '15px' }}>
            <div>
              <Avatar src={`${foto}/${selectedVisita?.foto}`} alt={selectedVisita?.nombre_completo} sx={{ width: 150, height: 150, marginRight: "15px", objectFit: "cover" }}/>
            </div>
            <div>
              <span><strong>Visitante:</strong> {selectedVisita?.nombre_completo}</span><p/>
              <span><strong>Empresa:</strong> {selectedVisita?.empresa} </span><p/>
              <span><strong>Vehiculo:</strong> {selectedVisita?.placa} </span>
            </div>
          </div>
          <p/>
          <div>
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select
                ...
                name="area_per"
                value={selectedCortina}
                onChange={handleDropdownChangeUpCortina}
                label="No. Cortina"
              >
                <MenuItem value={null}>SELECCIONAR CORTINA</MenuItem>
                {cortinas.map((item) => (
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              <TextField
                ...
                name="motivo" 
                label="Motivo de la visita"
                value={upPaq.motivo || selectedCortina?.area || ""}
                onChange={inputChangeUpPaq}
                inputProps={{ style: { textTransform: 'uppercase' } }}
              />
            </FormControl>
          </div>
        ...
        <DialogActions>
          <Button variant="outlined" color="secondary" onClick={handleCloseCortina} >
            Cancelar
          </Button>
          <Button variant="contained" color="primary" onClick={updateVisitaPaqueteria} >
            Guardar
          </Button>
        </DialogActions>
      </Dialog>
        ...
        <Box>
          <TableContainer component={Paper} sx={{ maxHeight: 440 }}>
            <Table>
              <TableHead>
                <TableRow >
                  <TableCell colSpan={8} align="left" style={{ backgroundColor: '#f5f3f3', textAlign:'center' }}><strong>VISITAS AGENDADAS (HOY)</strong></TableCell>
                </TableRow>
                  <TableCell align="center"></TableCell>
                  <TableCell align="left">Nombre</TableCell>
                  <TableCell align="left">Empresa</TableCell>
                ...
                {Array.isArray(visitasHoy) && visitasHoy.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} align="center">
                      No hay visitas registradas para hoy.
                    </TableCell>
                  </TableRow>
                    ...
                    <TableRow key={index}>
                      <TableCell align="center">
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Avatar 
                            src={visita.foto ? `${foto}/${visita.foto}` : '/placeholder.png'} 
                            alt={visita.nombre_completo || 'Sin nombre'} 
                            sx={{ width: 80, height: 80, marginRight: '15px', objectFit: 'cover' }} 
                          />
                        </div>
                      </TableCell>
                      <TableCell>{visita.nombre_completo}</TableCell>
                      <TableCell>{visita.id_catv === 2 ? 'No aplica' : visita.empresa}</TableCell>
                      <TableCell>{visita.personal}</TableCell>
                      <TableCell>{visita.area}</TableCell>
                      <TableCell>{formatDateToYMD(visita.reg_entrada)}, {visita.hora_entrada}</TableCell>
                      <TableCell>
                        {visita.placa ? visita.placa : 'No aplica'}
                        <br />
                        {visita.acc_veh === 'S' ? (
                          <small style={{ marginLeft: '10px' }}>Autorizado</small>
                        ) : (
                          <small>No autorizado</small>
                        )}
                      </TableCell>
                      <TableCell>
                        <IconButton color="secondary" onClick={() => cancelar(visita)}>
                          <Block />
                        </IconButton>
                      </TableCell>
                    </TableRow>
