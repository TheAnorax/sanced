// ✅ src/controllers/departamantalController.js
...
const getpick7066 = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: true, message: "Error al obtener los datos", details: error.message });
  }
};
...
const createPick7066 = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ error: false, message: "Registro creado", insertId: result.insertId });
  } catch (error) {
    console.error("❌ Error en createPick:", error);
    res.status(500).json({ error: true, message: "Error al crear el registro", details: error.message });
  }
};
...
const updatePick7066 = async (req, res) => {
  ...
  try {
    ...
    res.json({ error: false, message: "Registro actualizado", affectedRows: result.affectedRows });
  } catch (error) {
    console.error("❌ Error en updatePick:", error);
    res.status(500).json({ error: true, message: "Error al actualizar el registro", details: error.message });
  }
};
...
const deletePick7066 = async (req, res) => {
  const { id } = req.params;
  try {
    const [result] = await pool.query(`DELETE FROM departamental_pick WHERE id_ubicacion = ?`, [id]);
    res.json({ error: false, message: "Registro eliminado", affectedRows: result.affectedRows });
  } catch (error) {
    console.error("❌ Error en deletePick:", error);
    res.status(500).json({ error: true, message: "Error al eliminar el registro", details: error.message });
  }
};

module.exports = { getpick7066, createPick7066, updatePick7066, deletePick7066 };
