    ...
    if (autorizado === 1) {
      ...
      if (solRows.length > 0) {
        ...
        try {
          ...
          console.error(
            "❌ Error al leer plantilla o enviar correo:",
            errorCorreo
          );
        }
      } else {
...
const obtenerAutorizadas = async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT * FROM Solicitudes_muestras WHERE autorizado IN (1, 2)`
    );
    for (const row of rows) {
      const [productos] = await pool.query(
        `SELECT * FROM Carrito_muestras WHERE solicitud_id = ?`,
...
const registrarFinEmbarque = async (req, res) => {
  ...
  try {
    ...
    // Obtener datos de la solicitud
    const [rows] = await pool.query(
      "SELECT nombre, departamento, motivo FROM solicitudes_muestras WHERE folio = ?",
      [folio]
    );
    ...
    // Correos fijos a los que siempre se les manda copia
    const correosFijos = [
      "mauricio.sandler@santul.net",
      "eduardo.sandler@santul.net",
      "elias.sandler@santul.net",
    ...
    if (correoDestino) {
      ...
      await transporter.sendMail({
        from: `"Muestras Sanced" <tucorreo@santul.net>`,
        to: correoDestino,
        cc: correosFijos, // <-- Aquí agregamos los adicionales
        subject: `📦 Tus muestras están listas – Folio ${folio}`,
        html,
        attachments: [
          {
            filename: "logo_sanced.png",
    ...
    res.json({ message: "✅ Fin de embarque registrado y correo enviado." });
  } catch (error) {
