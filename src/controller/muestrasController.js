...
const correosPorNombre = {
  ...
  "Brenda Zuleyma Velazquez": "brenda.velazquez@santul.net",
...
const enviarPendientesEmbarque = async () => {
  ...
  WHERE fin_embarcado_at IS NULL
    ...
    AND autorizado = 1          -- SOLO AUTORIZADAS
    ...
    const filas = rows
      .map((r) => {
        ...
        <tr>
          ...
          <td style="padding:10px; border:1px solid #ccc; color:${
            diasRetraso > 2 ? "#EA0029" : "#333"
          }; font-weight:bold;">
...
const marcarSinMaterial = async (req, res) => {
  ...
  try {
    ...
    res.json({
      ok: true,
      message: "Solicitud marcada como sin material y correo enviado",
    });
  } catch (error) {
